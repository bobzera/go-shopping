        ...
        <div  className="" key={`${item.nameItem}`} >
            <div className="bg-gray-100 p-2 flex">
                <div className="flex items-center cursor-pointer" id={item.nameItem}  onClick={handleClick}>
                    ...
                    {!item.purchased  && <div className="w-5 h-5   bg-white border-2 rounded border-blue-900"></div>}
                    <h1 className={item.purchased ? "line-through pl-2 text-xl text-green-900" : "pl-2 text-xl text-blue-900"} >{item.nameItem}</h1>
                </div>
